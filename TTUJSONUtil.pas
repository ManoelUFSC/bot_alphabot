...
begin
  ...
  //
  //  // Converte para formato que È aceito pelo JSON. Ex.: "1.000,5" para "1000.5".
...
begin
  // JSON n„o possui formato especÌfico para data. Ent„o, deve ser convertido para string
...
begin
{$IFNDEF DELPHIXE}TTotallException.Create('Convers„o n„o disponÌvel!'); {$ENDIF}
...
{$IFDEF DELPHIXE}
    ...
    // Currency È armazenado como uma string porque o System.JSON sÛ trabalha com ponto como DecimalSeparator.
    if Assigned(pJSONObject) and (pJSONObject.GetValue(cParametro) is TJSONNumber) then begin
      ...
      //     cStr := StringReplace(StringReplace(cStr,',','',[rfReplaceAll]),'.',',',[rfReplaceAll]);
      //
      //     // Converte para formato numÈrico
...
{$IFDEF DELPHIXE}
    ...
    // Double È armazenado como uma string porque o System.JSON sÛ trabalha com ponto como DecimalSeparator.
    if Assigned(pJSONObject) and (pJSONObject.GetValue(cParametro) is TJSONNumber) then begin
      Result := TJSONNumber(pJSONObject.GetValue(cParametro)).AsDouble;
    end else begin
      Result := pDefault;
